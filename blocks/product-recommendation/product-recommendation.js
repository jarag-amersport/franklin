...
function renderItem(product, unitId) {
  ...
  container.querySelector('a').addEventListener('click', () => getMagentoStorefrontEvents((mse) => mse.publish.recsItemClick(unitId, product.productId)));

  return container;
}

function renderItems(block, unitId, products) {
  const grid = block.querySelector('.product-grid');

  grid.innerHTML = '';

  products.forEach((product) => {
    grid.appendChild(renderItem(product, unitId));
  });
}
...
async function loadRecommendations(block, config) {
  ...
  if (preconfigured && config.name) {
    unit = data?.results?.find((a) => a.unitName === config.name);
  ...
  if (status === 200 && unit && unit.products && unit.products.length > 0) {
    getMagentoStorefrontEvents((mse) => {
      mse.context.setRecommendations({ units: data?.results || data?.units });
      mse.publish.recsResponseReceived();
    });

    const productsWithImages = getImages(unit.products);
    renderItems(block, unit.unitId, productsWithImages);

    getMagentoStorefrontEvents((mse) => {
      mse.publish.recsUnitRender(unit.unitId);
  ...
  if (status === 200 && unit && unit.products && unit.products.length > 5) {
