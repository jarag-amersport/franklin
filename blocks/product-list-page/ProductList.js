...
class ProductCard extends Component {
  ...
  addToCart = async (product) => {
    const minicart = document.getElementById('my-mini-cart');
    minicart.dataset.sku = product.sku;
    minicart.dataset.image = product.images[0].url;

    const { loadPWA } = await import('../../scripts/pwa-dist/pwa-injector.js');
    await loadPWA('mini-cart');
  }
    ...
    return html`
      <li>
        <div class="picture">
          ...
          <button class="add-to-cart-action" onClick=${() => this.addToCart(product)}>Add to Bag</button>
        </div>
        <div class="variants">
...
const calculateDisruptorWidth = (baseWidth, size) => {
  const gap = 20 * (size - 1);
  return baseWidth * size + gap;
};

const renderDisruptorImage = (image, alt, size) => {
  const url = new URL(image, window.location.href);
  url.search = '';

  return html`<picture>
      <source type="image/webp" srcset="${url}?width=${calculateDisruptorWidth(163, size)}&bg-color=255,255,255&format=webply&optimize=medium" media="(max-width: 900px)" />
      <source type="image/webp" srcset="${url}?width=${calculateDisruptorWidth(330, size)}&bg-color=255,255,255&format=webply&optimize=medium" />
      <img src="${url}?width=${calculateDisruptorWidth(330, size)}&quality=100&bg-color=255,255,255" width="330" height="587" alt="${alt}" loading="lazy" />
  </picture>`;
};

const decorateDisruptor = (item) => {
  const image = renderDisruptorImage(item.Image, item.Name, item.Size);
  const content = item.Link ? html`<a href="${item.Link}">${image}</a>` : image;

  return html`<li style="grid-area: span 1 / span ${item.Size}">
    <div class="disruptor">
      ${content}
    </div>
  </li>`;
...
const ProductList = ({
  products, loading, currentPageSize, disruptors,
}) => {
  if (loading) {
  ...
  if (disruptors) {
    const disruptorItems = disruptors.map(
      (item, index) => ({position: item.Position, element: decorateDisruptor(item, index)}),
    ).sort((a, b) => a.position - b.position);
    disruptorItems.forEach((disruptorItem, index) => {
      gridItems.splice(disruptorItem.position - 1 + index, 0, disruptorItem.element);
    });
  }
