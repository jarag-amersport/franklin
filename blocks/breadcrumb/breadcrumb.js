import {
  getCategoryNameFromUrlKey, isPDP, wrapWithSup, getMagentoStorefrontEvents,
...
async function createPDPBreadcrumb(block) {
  const possiblePLPs = await getCategoryNameFromUrlKey();
  const category = possiblePLPs[0];
  ...
  if (category) {
    ...
    const urlKeys = [...new Set(possiblePLPs.map((item) => item.url_key))].map((key) => `${key}|maidenform|us`);
    const localStorageData = localStorage.getItem('productInfo');
    if (localStorageData) {
      const productInfo = JSON.parse(localStorageData);
      productInfo.plp.categories = urlKeys;
      getMagentoStorefrontEvents(async (mse) => {
        mse.context.setProduct(productInfo);
