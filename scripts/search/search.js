...
import { renderFallbackImage, getMagentoStorefrontEvents } from '../commerce.js';
...
class SearchBox extends Component {
  ...
  handleSearchSubmit = () => {
    const inputElement = this.searchBarElement.querySelector('input[name=q]');
    if (inputElement) {
      const numberOfResults = this.state.searchResults.topResults.length;
  ...
  componentDidMount() {
    ...
    const buttonElement = this.searchBarElement.querySelector('.search-button');
    if (buttonElement) {
      buttonElement.addEventListener('click', this.handleSearchSubmit);
    }
  }
  ...
  componentWillUnmount() {
    ...
    const buttonElement = this.searchBarElement.querySelector('.search-button');
    if (buttonElement) {
      buttonElement.removeEventListener('click', this.handleSearchSubmit);
    }
  }
