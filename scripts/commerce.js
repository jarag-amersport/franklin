...
const PWA_BROWSERPERSISTENCE_SIGNIN_TOKEN = 'M2_VENIA_BROWSER_PERSISTENCE__signin_token';
const PWA_BROWSERPERSISTENCE_DETAILS = 'M2_VENIA_BROWSER_PERSISTENCE__user_details';
...
export function getPWABrowserPersistenceItem(itemType, loggedOutCallback) {
  const now = Date.now();
  const item = window.localStorage.getItem(itemType);
  if (!item) {
...
export function getSignInToken(loggedOutCallback) {
  return getPWABrowserPersistenceItem(PWA_BROWSERPERSISTENCE_SIGNIN_TOKEN, loggedOutCallback);
}

export function getUserDetails(loggedOutCallback) {
  return getPWABrowserPersistenceItem(PWA_BROWSERPERSISTENCE_DETAILS, loggedOutCallback);
}

/**
 *
 * @param {string} query The query
 * @param {object} variables The query variables as an object
 * @param {boolean} GET Whether to use the GET HTTP method
 * @param {string|boolean} USE_TOKEN When passed as a boolean, the method will decide whether to get
 * an auth token. If passed as a string, this will be used as the auth token.
 * @returns {Promise<null|any>}
 */
export async function performMonolithGraphQLQuery(query, variables, GET = true, USE_TOKEN = false) {
  const GRAPHQL_ENDPOINT = await getConfigValue('graphql-endpoint');
...
export function getPageName() {
  ...
  const documentLocation = document?.location?.pathname?.split('/').pop()?.replace(/\.html$|\.htm$/i, '');
  const locationHref = window.location?.href?.split('/').pop()?.replace(/\.html$|\.htm$/i, '');

  return `maidenform:${documentTitle || documentLocation || locationHref || 'Unspecified'}`;
}
...
async function loadDisruptors(category, currentPage) {
  const disruptorsScript = await import('./disruptors.js');
  return disruptorsScript.default(category, currentPage);
}
...
export async function loadCategory(state) {
  try {
    ...
    let response = performCatalogServiceQuery(productSearchQuery, variables);
    let disruptors = loadDisruptors(state.category.id, state.currentPage);
    ([response, disruptors] = await Promise.all([response, disruptors]));
...
export async function performCommerceChecks() {
  const { processCommerceCookies } = await import('./commerce-coupons.js');
...
export async function loadCategoryInfo(url, referrerUrl) {
  try {
    ...
    if (pathSegments.length < 1) {
      throw new Error('URL does not have the expected format.');
    }

    const categoryUrlKey = pathSegments.pop();
    const siteSection = pathSegments.shift();

    const siteSectionLevel2 = pathSegments.length > 0 ? pathSegments.shift() : '';
    const siteSectionLevel3 = pathSegments.length > 0 ? pathSegments.shift() : '';
    const siteSectionLevel4 = pathSegments.length > 0 ? pathSegments.shift() : '';

    const productCategory = `${categoryUrlKey}|maidenform|us`;

    let previousPageName = '';
    // Compute previousPageName regardless of whether an error occurred
    try {
      const a = document.createElement('a');
      a.href = referrerUrl;
      previousPageName = a.pathname.substring(a.pathname.lastIndexOf('/') + 1);
    } catch (error) {
      console.error(`Error while parsing referrer URL: ${error.message}`);
    }
    ...
    const categoryInfo = {
      ...
      pageType: 'plp',
      previousPageName,
      storeCode: 'maidenform_store_view',
    };
    return categoryInfo;
  } catch (error) {
