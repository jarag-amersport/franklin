...
import {
  preloadLCPImage, preloadCategory, isPDP, performCommerceChecks,
} from './commerce.js';
...
async function loadLazy(doc) {
  ...
  loadCSS('//use.typekit.net/rus5dnv.css');
...
function loadDelayed() {
  if (new URLSearchParams(window.location.search).get('skip_delayed') !== 'true') {
    // eslint-disable-next-line import/no-cycle
    window.setTimeout(() => import('./delayed.js'), isCategoryPage ? 5000 : 3000);
...
export function getTimestamp() {
  ...
  if ((environment === 'dev' || environment === 'stage') && window.sessionStorage.getItem('preview-date')) {
