...
import DEFAULT_OPERATIONS from '@magento/peregrine/lib/talons/CartPage/ProductListing/EditModal/productForm.gql';
    ...
    const handleSubmit = useCallback(
        async formValues => {
            try {
                ...
                const selectedOptionsUID = 
                    selectedVariant?.attributes?.map(({ value_index }, i) => {
                        const current = cartItem.configurable_options[i];
                ...
                await handleSubmitExternal(cartItem.sku, selectedOptionsUID, quantity)
