...
export const useCouponCode = props => {
  ...
  const [couponLib, setCouponLib] = useState(null);
  const [couponCookie, setCouponCookie] = useState(null);

  useEffect(() => {
    const loadCouponLib = async () => {
      const loadedCouponLib = await import('../../../../../scripts/commerce-coupons.js');
      const {getCookie, COUPON_CODE_COOKIE_NAME} = loadedCouponLib;
      const gotCouponCookie = getCookie(COUPON_CODE_COOKIE_NAME);
      setCouponLib(loadedCouponLib);
      setCouponCookie(gotCouponCookie);
    }
    loadCouponLib()
  }, [])
  ...
  const handleRemoveCoupon = useCallback(
    async couponCode => {
      if(couponLib){
      const {deleteCookie, COUPON_CODE_COOKIE_NAME} = couponLib;
      try {
        await removeCoupon({
          variables: {
            cartId,
        ...
        console.info("deleting coupon cookie");
        deleteCookie(COUPON_CODE_COOKIE_NAME);
      } catch (e) {
        // Error is logged by apollo link - no need to double log.
      }
      }
    },
    [couponLib, cartId, removeCoupon]
  );
      ...
      const applyCookie = async () => {
        try {
          await applyCoupon({
            variables: {
              cartId,
              couponCode: couponCookie
            }
          });
          console.log('coupon applied from cookie')
        } catch (e) {
          console.error('cannot apply coupon from cookie', e)
        }
        console.info("deleting coupon cookie")
        deleteCookie(COUPON_CODE_COOKIE_NAME);
      }
