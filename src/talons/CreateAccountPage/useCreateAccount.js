  ...
  const handleSubmit = useCallback(
    async formValues => {
      ...
      try {
        ...
        // Add customer address to account
        const region = JSON.parse(formValues.customer.state);
        await addAddress({
          variables: {
            firstname: formValues.customer.firstname,
            lastname: formValues.customer.lastname,
            street: [formValues.customer.streetAddress1, formValues.customer.streetAddress2].filter((x) => !!x),
            city: formValues.customer.city,
            region: region.region,
            region_code: region['region_code'],
            region_id: region['region_id'],
            postcode: formValues.customer.zip,
            telephone: formValues.customer.phonenumber,
          }
        });
